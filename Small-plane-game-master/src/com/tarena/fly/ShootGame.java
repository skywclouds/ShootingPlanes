...
public class ShootGame extends JPanel {
	...
	public static final int HEIGHT = 620; // 面板高
	...
	private int intervel = 20; // 时间间隔(毫秒)
	...
	public void action() {
		...
		MouseAdapter l = new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) { // 鼠标点击
				if(have) {
					frame.setSize(WIDTH, HEIGHT+230);
					frame.setLocationRelativeTo(null); // 设置窗体初始位置
					have=false;
				}
				switch (state) {
	...
	public void checkGameOverAction() {
		if (isGameOver()) {
			URL url=getClass().getResource("/res/游戏失败.wav");
	...
	public boolean isGameOver() {
		...
		for (int i = 0; i < flyings.length; i++) {
			...
			if (hero.hit(obj)) { // 检查英雄机与飞行物是否碰撞
				URL url=getClass().getResource("/res/爆炸声.wav");
				AudioClip au=Applet.newAudioClip(url);
				au.play();
	...
	public static FlyingObject nextOne() {
		if (Math.random()<0.1) {//有10%的概率生成蜜蜂
			return new Bee();
		} else {
			return new Airplane();
		}
